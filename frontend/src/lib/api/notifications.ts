...
export async function getBinnedNotifications(
  signal?: AbortSignal,
): Promise<Notification[]> {
  const { data } = await api.get<Notification[]>("/api/v1/notifications/bin", {
    signal,
  });
  return data;
}
...
export async function deleteAllNotifications(): Promise<NotificationDeleteCount> {
  const { data } = await api.delete<NotificationDeleteCount>(
