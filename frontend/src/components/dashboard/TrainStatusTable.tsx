...
export default function TrainStatusTable() {
  ...
  const rows = useMemo<TrainRow[]>(() => {
    ...
    return (schedules.data ?? []).map((schedule) => {
      ...
      return {
        ...
        // same `is_active` flag the "Active Trains" KPI card counts, so
        // this filter matches that number's definition of "active"
  ...
  const filtered = rows
    .filter((row) => (statusFilter === "active" ? row.isActive : true))
    .filter((row) =>
  ...
  // Each row here is one scheduled stop, not one train - a single active
  // train can have several stops/stations in the list. This count is the
  // distinct number of trains behind those rows, so it matches what the
  // "Active Trains" KPI card up top is counting.
  const activeTrainCount = useMemo(
    () => new Set(rows.filter((row) => row.isActive).map((row) => row.trainNumber)).size,
    [rows],
  );
      ...
      <div className="mb-8 flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div>
          ...
          <p className="mt-2 text-muted">
            {statusFilter === "active"
              ? `${activeTrainCount} active train${activeTrainCount === 1 ? "" : "s"} \u00b7 ${filtered.length} scheduled stop${filtered.length === 1 ? "" : "s"} shown`
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <div className="flex items-center gap-2 rounded-xl border border-border p-1">
            ...
            {(
              [
                { key: "active", label: "Active Trains" },
              ...
              {search
                ...
                : delayFilter === "on_time"
                  ...
                  : delayFilter === "delayed"
                    ...
                    : statusFilter === "active"
                      ? "No active trains right now. Try \"All Trains\"."
                      : "No trains found."}
            </p>
          ) : (
            <List
