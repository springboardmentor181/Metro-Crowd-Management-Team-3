...
export default function LiveStationMonitor() {
  ...
  const filtered = useMemo(() => {
    const byLevel = level === "all" ? merged : merged.filter((s) => s.crowd_level === level);
    // Hide stations with no passenger flow data at all (both inflow and
    // outflow are 0) - keeps the table focused on stations actually
    // reporting movement.
    return byLevel.filter((s) => s.inflow > 0 || s.outflow > 0);
  }, [merged, level]);
