...
import { projectPoints } from "@/lib/mapProjection";
...
export default function MetroMap() {
  ...
  const { lines, stationCount, totalInCity } = useMemo(() => {
    const inCity = (allStations ?? []).filter((s) => s.city === activeCity);
    const crowdById = new Map((crowdList ?? []).map((c) => [c.station_id, c]));

    // Rank-based placement + decluttering (same approach as Live Train
    // Tracking) instead of raw linear lat/lng scaling. A real metro
    // network isn't evenly spread - a dense city-centre cluster plus a
    // handful of far-flung outliers - so linear min/max scaling
    // stretched the box to fit those outliers and squashed almost every
    // station into one tiny corner (the near-empty map with just a
    // couple of long diagonal lines).
    const positioned: PositionedStation[] = projectPoints(inCity).map((s) => {
      ...
      return { ...s, current_count, crowd_level };
    });
    ...
    return { lines: lineList, stationCount: positioned.length, totalInCity: inCity.length };
        ...
        <div className="xl:col-span-2">
          ...
          >
            ...
            {!stationsLoading && stationCount === 0 && (
              <p className="absolute inset-0 flex items-center justify-center px-6 text-center text-muted">
                {totalInCity > 0
                  ? `${totalInCity} station${totalInCity === 1 ? "" : "s"} found for ${activeCity}, but none has valid map coordinates (latitude/longitude missing or 0,0).`
