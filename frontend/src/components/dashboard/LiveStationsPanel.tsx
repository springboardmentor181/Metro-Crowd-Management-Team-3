...
export default function LiveStationsPanel() {
  ...
  const { data: crowd, lastFetchedAt: crowdFetchedAt } = useApiData(
    queryKeys.crowdDashboard,
    (signal) => getCrowdDashboard(selectedState ?? undefined, signal),
    [selectedState],
    // Bugfix: this used to be `isConnected ? 0 : 30000` - fully disabling
    // REST polling whenever the socket reports "connected", on the
    // assumption that crowd_update pushes would keep data fresh instead.
    // But `isConnected` only reflects the WebSocket *transport* being up -
    // it says nothing about whether the backend's crowd simulator is
    // actually running (it's a separate, manually-started admin job; see
    // app/simulator/scheduler.py::is_simulator_running). If that job is
    // off or stalls, the socket stays "connected" while genuinely never
    // sending another crowd_update again, and with polling fully off too
    // there was no path left to ever refresh - which is exactly why the
    // "updated Xs ago" indicator could sit frozen for hours, climbing on
    // nothing but its own local per-second tick. Always keeping this
    // 30s safety-net poll running means the dashboard self-heals from a
    // quiet/dead simulator instead of depending entirely on the socket.
    30000,
  );
    ...
    crowd_update: (payload) => {
      ...
      setLiveById((prev) => {
        ...
        for (const u of payload.updates) {
          // Always refresh `at`, even when count/level are unchanged from
          // the previous push - a socket message still means "the server
          // just confirmed this station is current", and the "updated Xs
          // ago" display should reset on that, not only on a value change.
          next[u.station_id] = {
  ...
  const cards = useMemo(() => {
    // Prefer the live socket push's own timestamp when we have one - it's
    // the freshest signal. Otherwise, fall back to when we last actually
    // synced with the server (crowdFetchedAt), not the raw `last_updated`
    // field from that payload: that field only moves when the underlying
    // count/level changes, so it can sit hours in the past even though we
    // just polled and confirmed the value is still correct. Using our own
    // poll time keeps the "updated Xs ago" indicator honest and ticking
    // (1s, 2s, 3s...) instead of jumping straight to a huge stale number.
    const fetchedAtIso = crowdFetchedAt ? new Date(crowdFetchedAt).toISOString() : null;
    return (stations ?? []).map((s) => {
      ...
      return {
        ...
        lastUpdated: live?.at ?? (base ? fetchedAtIso ?? base.last_updated : null),
      };
    });
  }, [stations, crowdById, liveById, crowdFetchedAt]);
