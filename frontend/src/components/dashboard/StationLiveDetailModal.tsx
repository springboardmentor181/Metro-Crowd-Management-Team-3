...
export default function StationLiveDetailModal({ station, onClose }: Props) {
  const { setFocusedStation } = useFocusedStation();
  ...
  // Focus the heatmap on the clicked station instead of showing the
  // generic all-stations heatmap - and clear it again on close/unmount
  // so the main dashboard's heatmap isn't left focused afterwards.
  useEffect(() => {
    setFocusedStation({
      id: station.id,
    ...
    return () => setFocusedStation(null);
  }, [station, setFocusedStation]);
