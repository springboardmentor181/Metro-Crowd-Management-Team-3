  ...
  return (
    ...
    >
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex h-9 shrink-0 items-center gap-1.5 rounded-xl border border-border bg-card px-2.5 text-sm font-semibold transition hover:border-primary sm:h-12 sm:gap-2 sm:rounded-2xl sm:px-4"
      >
        <MapPin
          size={18}
      ...
      {open && (
        <div className="absolute right-0 z-50 mt-2 max-h-96 w-72 max-w-[90vw] overflow-y-auto rounded-2xl border border-border bg-card p-2 shadow-2xl">
